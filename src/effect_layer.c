...
static void effect_layer_update_proc2(Layer *me, GContext* ctx) {
  ...
  // Call the function that swaps in our desired colors a and b:
  effect_double_colorize(ctx, layer_frame, color_a, color_b);
}  
...
EffectLayer* effect_layer_create(GRect frame) {
  ...
  layer_set_update_proc(layer, effect_layer_update_proc2);
